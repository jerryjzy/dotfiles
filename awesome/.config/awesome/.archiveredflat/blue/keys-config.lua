...
local focus_switch_byidx = function(idx)
	return function()
...
local move_client_byd = function(dir)
	return function()
...
-- move window by direction
local move_client_byidx = function(idx)
	return function()
		awful.client.swap.byidx(idx)
		if client.focus then client.focus:raise() end
	end
end
...
function hotkeys:init(args)
	...
	self.raw.root = {
		...
		{
			{ env.mod }, "j", focus_switch_byidx(1),
			{ description = "Go to next client", group = "Client focus" }
		},
		{
			{ env.mod }, "k", focus_switch_byidx(-1),
		...
		-- },
		{
			{ env.mod, "Shift"}, "j", move_client_byidx(1),
			{ description = "Swap with next client", group = "Client focus" }
		},
		{
			{ env.mod, "Shift"}, "k", move_client_byidx(-1),
			{ description = "Swap with previous client", group = "Client focus" }
		},
		-- {
			-- { env.mod, "Shift"}, "l", move_client_byd("right"),
			-- { description = "Swap with right client", group = "Client focus" }
		-- },
		-- {
			-- { env.mod, "Shift" }, "h", move_client_byd("left"),
			-- { description = "Swap with left client", group = "Client focus" }
		-- },
		-- {
			-- { env.mod, "Shift" }, "k", move_client_byd("up"),
			-- { description = "Swap with upper client", group = "Client focus" }
		-- },
		-- {
			-- { env.mod, "Shift" }, "j", move_client_byd("down"),
